  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {guide.sections.map((section) => {
        ...
        return (
          <section key={section.id} className="overflow-hidden border border-border bg-card">
            <button
              type="button"
              aria-expanded={isOpen}
              onClick={() => toggleSection(section.id)}
              className="flex w-full flex-col gap-2 border-b border-border bg-muted/40 px-4 py-3 text-left transition-colors hover:bg-muted/60 md:flex-row md:items-center md:justify-between"
            >
              <span className="flex items-center gap-2">
                <ChevronDown
                  className={`size-4 text-muted-foreground transition-transform ${isOpen ? "rotate-0" : "-rotate-90"}`}
                />
                    ...
                    {section.rows.map((row) => {
                      ...
                      return (
                        <tr key={row.id} className="border-t border-border align-top">
                          ...
                          <td className="max-w-xl px-3 py-3 text-muted-foreground">{row.notes || ""}</td>
                        </tr>
                      );
                    })}
                  </tbody>
